...
| `VITE_DEMO` | `true` → backend olmadan yaddaşdakı nümunə data ilə işləyir. `false` → real API |
...
Başqa heç nə dəyişmir. Kodda `if (demo)` şərti yoxdur — demo rejimi axios-un
adapter səviyyəsində dəyişir, yuxarıdakı bütün kod (klient, interceptor-lar,
token yeniləmə, xəta emalı, ekranlar) eyni qalır.
...
`VITE_DEMO=true` olanda axios sorğuları şəbəkə əvəzinə yaddaşdan cavablandırır.
Adapter API-nin müşahidə oluna bilən hissəsini təkrarlayır: cavab zərfi,
səhifələmə, hər resursun filtr və sıralama siyahısı, rol qaydaları, sürücü
məhdudlaşdırması, kaskadlı soft delete və dashboard hesablamaları.

Demo hesabları (login ekranında da göstərilir):

| İstifadəçi | Şifrə | Rol |
|---|---|---|
| `admin` | `Admin123!` | Admin |
| `menecer` | `Menecer123!` | Manager |
| `surucu` | `Surucu123!` | Driver (sürücü kartına bağlı) |

**Demo rejimi biznes qaydaları üçün ikinci həqiqət mənbəyi deyil.** Adapter ilə
API arasında fərq olsa, **API haqlıdır**. Onun işi backend olmadan interfeysi
açıb baxa bilməkdir.
...
drivious-web/src/
  api/          Klient, endpoint-lər, DTO tipləri, mesaj tərcüməsi
    demo/       Yaddaşdakı adapter və nümunə filo
...
1. Demo rejimində səhifə yeniləndikdə sessiya itirdi — tokenlər yalnız yaddaş
   `Map`-ində idi. İndi hesab ID-si tokenin özündədir (rotasiya qorunub).
...
- [ ] `.env`-də `VITE_DEMO=false` edib real API ilə uçdan-uca sına
