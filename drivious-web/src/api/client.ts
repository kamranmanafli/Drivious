...
import { demoAdapter, DEMO } from "./demo/adapter";
...
export const api = axios.create({
  baseURL,
  // In demo mode every request is answered from memory instead of the network,
  // so the interceptors, error shapes and refresh flow below are still the ones
  // exercised — the only thing that changes is where the bytes come from.
...
async function refreshAccessToken(): Promise<string> {
  ...
  const { data } = await axios.post<ApiResponse<{ accessToken: string; refreshToken: string }>>(
    ...
    { refreshToken },
    DEMO ? { adapter: demoAdapter } : undefined,
  );
...
export function errorMessage(error: unknown): string {
  ...
  if (axios.isAxiosError(error)) {
    ...
    if (error.code === "ERR_NETWORK") {
      return `Serverə qoşulmaq mümkün olmadı (${baseURL}). API işləyirmi?`;
    }
